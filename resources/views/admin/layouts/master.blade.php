...
<footer class="footer footer-static footer-light">
    ...
    {{-- <button class="test_notification btn btn-primary">test</button> --}}
...
<div class="modal fade text-start" id="send_whats" tabindex="-1" aria-labelledby="myModalLabel1" aria-hidden="true">
    <div class="modal-dialog">
        <form id="whatsFormss" method="post" action="#">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel1">{{ __('admin.dialogs.whats.title') }}</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div id="whatsloader" class="col-md-12 text-center" >
                        جارى حساب الارقام التى سيتم الارسال لها
                        <br>
                        <br>
                        <img src="{{ asset('loading-gif.gif') }}" alt="" height="100px">
                    </div>
                    <div id="whatsRemain" style="display: none">

                        <h3>سيتم الارسال الى <span id="remainNumber"></span> رقم</h3>
                        {{ __('admin.dialogs.whats.info') }}
                    </div>

                </div>
                <div class="modal-footer" id="whatsfooter" style="display: none">
                    <button type="submit" class="btn btn-sm btn-success">{{ __('admin.dialogs.whats.confirm') }}</button>
...
<script>
    $(window).on('load', function() {
        ...
        $('body').on('click', '.delete_item', function (){
         ...
         $('body').on('click', '.send_whats', function (){
            $('#remainNumber').text('');
            $('#whatsloader').show();
            $('#whatsRemain').hide();
            $('#whatsfooter').hide();
            ...
            var percent2= $(this).attr('data-percent2');
            var category_id= $(this).attr('data-category_id');
            var type= $(this).attr('data-type');
            $('#whatsFormss').attr('action', url)
        ...
        $(document).on('change', '#main_archive_id', function(){
    ...
    function setCookie(cvalue) {
        ...
        document.cookie =  "mood=" + cvalue + ";" + expires + ";path=/";
    }
    function getCookie() {
        let name = "dataLayout";
  ...
  <div class="toast-container" style="display: none">
        ...
        >
            <div class="toast-header">
                ...
                <strong class="me-auto">{{__("admin.notify")}}</strong>
                <button type="button" class="ms-1 btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
            </div>
            <div class="toast-body">تم التحديث بنجاح</div>
        </div>
    </div>
</body>
<!-- END: Body-->
